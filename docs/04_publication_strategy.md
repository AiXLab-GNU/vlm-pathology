...
- 재현 스크립트: `models/pilot_vlm_benchmark_{gpt5,claude,quilt,llavamed}.py`,
  원본 로그 `models/vlm_benchmark_cache/`.

### 2) 통계적 엄밀성 — ✅ 완료 (2026-07-28)
`models/pilot_statistical_corrections.py`로 13개 마커 가설 전부에 대해 일관된 프로토콜로
...
- **(임상변수만) vs (이미지만) vs (임상변수+이미지) 비교, ΔAUROC/ΔR², LRT**: ✅ 완료
  (`models/pilot_confounder_audit.py`, 2026-07-31). 마커④PTEN: 결합 모델이 양쪽
...
- **grade-내부 permutation 검정**: ✅ 완료(`models/pilot_confounder_permutation.py`,
...
- **TCGA 다중 assay 분자적 일치성**: ✅ 완료(`models/pilot_tcga_multiassay_concordance.py`,
...
- **LEOPARD** (`leopard.grand-challenge.org`): 공식 학습셋 **508명 단일 기관
  ...
  - **✅ 데이터 확보 완료(2026-07-31)**: grand-challenge.org 등록이 실제로는
    ...
    event=1(재발) 87. `opendataset/LEOPARD/training_labels.csv`로 저장 완료.
  ...
  - **⚠️ Publication embargo 발견(2026-07-31, 4차례 외부 검토에서도 안 걸러짐)**:
    ...
    문의해 재확인한다.** 초안(`opendataset/LEOPARD/organizer_email_draft.txt`)이 이미
