...
import pandas as pd
import tifffile
from models import aggregate_stability_grid as aggregation

from models.aggregate_stability_grid import (
...
FULL_SPEC = ROOT / "models/stability_grid_spec.csv"
FULL_RUN_ROOT = ROOT / "models/stability_runs/full"
...
class PublicationTests(unittest.TestCase):
    ...
    def test_manifest_records_eight_nonself_output_sizes_and_mtimes(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            output_dir, figure_dir = root / "models", root / "figures"
            frames = self.publication_frames(root)
            with (
                patch("models.aggregate_stability_grid._enumerate_declared_inputs",
                      return_value=frames["raw_input_paths"]),
    ...
    def test_cli_defaults_are_strict_and_weakening_flags_are_explicit(self):
        defaults = parse_args([])
        self.assertEqual(defaults.spec, Path("models/stability_grid_spec.csv"))
        self.assertEqual(defaults.fold_assignments, Path("models/stability_fold_assignments.csv"))
        self.assertEqual(defaults.run_root, Path("models/stability_runs/full"))
        self.assertEqual(defaults.log_root, Path("models/stability_runs/logs"))
        self.assertEqual(defaults.output_dir, Path("models"))
        self.assertEqual(defaults.figure_data_dir, Path("paper/figure_data"))
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with (
                ...
                with self.assertRaisesRegex(StabilityGridIntegrityError, "QC assertion"):
                    run_aggregation(
                        root / "spec", root / "assign", root / "runs", root / "logs",
                        root / "models", root / "figures", scan_tiffs=False,
                        require_full_grid=False,
                    )
            self.assertFalse((root / "models/stability_run_manifest.csv").exists())
    ...
    def test_malformed_raw_failure_creates_no_approved_output(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            approved = aggregation._output_targets(root / "models", root / "figures")
            self.assertFalse(any(path.exists() for path in approved.values()))
            self.assertFalse((root / "models/.stability-aggregation.lock").exists())
    ...
    def test_full_repository_grid_reconciles_without_writing(self):
        ...
        representative_inputs = [
            ...
            ROOT / "models/stability_runs/logs/tcga_conch.log",
            representative_tiff,
        ]
        ...
        result = build_all_frames(
            ...
            ROOT / "models/stability_fold_assignments.csv",
            FULL_RUN_ROOT,
            ROOT / "models/stability_runs/logs",
            scan_tiffs=True,
