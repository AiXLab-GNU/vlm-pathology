...
from paper import build_revision_package as builder
...
class RevisionFinalPackageTests(unittest.TestCase):
    ...
    def test_temporary_build_covers_active_assets_without_mutating_repository(self):
        before = {
            ...
            for path in (
                ROOT / "paper/figure_manifest.csv",
                ROOT / "paper/table_manifest.csv",
                ROOT / "paper/main.pdf",
                ROOT / "paper/supplement_main.pdf",
            )
            if path.is_file()
        }
    ...
    def _numeric_stage(root: Path) -> Path:
        ...
        shutil.copy2(ROOT / "paper/sections/results.tex", results)
    ...
    def test_numeric_qa_maps_each_occurrence_to_the_intended_source_field(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            frame = pd.read_csv(ROOT / row["source_path"])
    ...
    def test_package_status_is_shared_by_actions_report_and_return_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            actual = builder.determine_package_status(ROOT / "paper/author_action_items.md")
    ...
    def test_table_registry_covers_all_active_submission_tables(self):
        ...
        self.assertEqual(by_id["S3"].sources, ("models/stability_summary.csv",))
    ...
    def test_evidence_axis_table_requires_complete_source_linked_grid(self):
        from paper.generate_submission_tables import render_evidence_axis_audit

        matrix = pd.read_csv(ROOT / "paper/figure_data/evidence_axis_matrix.csv")
        claims = pd.read_csv(ROOT / "paper/claim_evidence_matrix.csv")
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_multiplicity_renderer_requires_exact_schema_keys_and_source_order(self):
        from paper.generate_submission_tables import _display_test, render_multiplicity_family

        source = pd.read_csv(ROOT / "models/revision_global_fdr_summary.csv")
        expected_columns = (
    ...
    def test_stability_renderer_recomputes_exact_six_by_twelve_summary(self):
        from paper.generate_submission_tables import render_stability_summary

        source = pd.read_csv(ROOT / "models/stability_summary.csv")
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_stale_output_is_rejected_by_manifest_validation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source = ROOT / manifest.iloc[0]["source_paths"].split(";")[0]
    ...
    def test_table_generator_fails_closed_on_missing_semantic_row(self):
        from paper.generate_submission_tables import render_qualification_summary
        source = pd.read_csv(ROOT / "paper/claim_evidence_matrix.csv").iloc[:-1]
        with tempfile.TemporaryDirectory() as tmp:
