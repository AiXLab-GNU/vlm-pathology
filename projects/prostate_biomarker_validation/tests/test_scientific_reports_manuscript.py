...
from projects.prostate_biomarker_validation.paper.manuscript_contract import (
    count_prose_words,
    extract_environment,
...
from projects.prostate_biomarker_validation.paper.submission_config import MAIN_FIGURES
...
def _repo_path(relative: str) -> Path:
    ...
    if path.parts and path.parts[0] == "paper":
        return PAPER.joinpath(*path.parts[1:])
    return ROOT / path
...
def _rows(relative: str) -> list[dict[str, str]]:
    with _repo_path(relative).open(encoding="utf-8", newline="") as handle:
...
def _pdf_text(relative: str) -> str:
    ...
    return subprocess.run(
        (mutool, "draw", "-F", "txt", "-o", "-", str(_repo_path(relative))),
        check=True,
        capture_output=True,
        text=True,
    ).stdout
...
class SubmissionDraftContractTests(unittest.TestCase):
    ...
    def test_declaration_bodies_are_explicit_and_not_placeholders(self) -> None:
        ...
        self.assertIn("National Research Foundation of Korea", declarations)
        self.assertIn("NRF-2023R1A2C1006639", declarations)
        self.assertIn("Institute of Information \\& Communications Technology", declarations)
        self.assertIn("IITP-2026-RS-2024-00436773", declarations)
        self.assertIn("Gyeongsang National University", declarations)
    ...
    def test_code_availability_names_the_public_repository_and_entry_points(self) -> None:
        ...
        for path in (
            "resources/projects/prostate_biomarker_validation/model_workspace/build_revision_p0_artifacts.py",
            "resources/projects/prostate_biomarker_validation/model_workspace/aggregate_stability_grid.py",
            "resources/projects/prostate_biomarker_validation/model_workspace/build_tcga_cdr_pfi_evidence.py",
            "resources/projects/prostate_biomarker_validation/model_workspace/build_marker7_survival_paired_analysis.py",
            "resources/projects/prostate_biomarker_validation/model_workspace/run_marker7_common_source_sensitivity.py",
            "resources/projects/prostate_biomarker_validation/model_workspace/build_ar_spop_evidence_closure.py",
        ):
    ...
    def test_stability_counts_distinguish_seed_ranges_from_t_intervals(self) -> None:
        results = _read("paper/sections/results.tex")
        summary = _rows("resources/projects/prostate_biomarker_validation/model_workspace/stability_summary.csv")
