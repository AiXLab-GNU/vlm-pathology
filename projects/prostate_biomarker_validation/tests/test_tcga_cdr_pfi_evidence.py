...
SCRIPT = ROOT / "resources/projects/prostate_biomarker_validation/model_workspace/build_tcga_cdr_pfi_evidence.py"
REAL_INPUTS = all(
    (ROOT / path).is_file()
    for path in (
        "resources/data/prostate_biomarker_validation/local-data/TCGA-CDR/TCGA-CDR-SupplementalTableS1.xlsx",
        "resources/data/prostate_biomarker_validation/local-data/TCGA-CDR/PanCan-Clinical_Open_GDC-Manifest_1.txt",
        "resources/data/prostate_biomarker_validation/local-data/TCGA-CDR/source_provenance.json",
        "resources/projects/prostate_biomarker_validation/model_workspace/confounder_nested_predictions.csv",
        "resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_clinical_extra/prad_pancan_clinical.json",
        "resources/data/shared/opendataset/TCGA-PRAD-BCR/bcr_recurrence_only.csv",
    )
)
                ...
                with patch.object(
                    ...
                    with self.assertRaisesRegex(evidence.EvidenceError, "canonical"):
                        evidence.run_analysis(
                            ROOT, ROOT / "resources/projects/prostate_biomarker_validation/model_workspace", n_boot=draws, seed=seed
                        )
    ...
    def test_official_run_reconciles_counts_hashes_endpoints_and_manifest(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            table_s1 = pd.read_excel(
                ROOT / "resources/data/prostate_biomarker_validation/local-data/TCGA-CDR/TCGA-CDR-SupplementalTableS1.xlsx",
                sheet_name="TCGA-CDR",
            )
