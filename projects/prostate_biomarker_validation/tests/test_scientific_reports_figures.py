...
FIGURE_DATA = ROOT / "projects/prostate_biomarker_validation/paper" / "figure_data"


class _CanonicalImportlib:
    """Keep historical test IDs while importing from the canonical project package."""

    @staticmethod
    def import_module(name, package=None):
        if name.startswith("paper.figures."):
            name = "projects.prostate_biomarker_validation." + name
        return _stdlib_importlib.import_module(name, package)


importlib = _CanonicalImportlib()
...
RENDERER_CONTRACTS = (
    ...
    (
        ...
        (ROOT / "resources/projects/prostate_biomarker_validation/model_workspace" / "pfi_endpoint_concordance.csv",),
        (0, "event_agreement"),
...
class SubmissionRendererTests(unittest.TestCase):
    ...
    def test_sfig4_evidence_axis_matrix_is_complete_and_deterministic(self):
        ...
        sources = (
            ...
            ROOT / "projects/prostate_biomarker_validation/paper/claim_evidence_matrix.csv",
        )
