...
\texttt{resources/projects/prostate_biomarker_validation/model_workspace/pfi\_endpoint\_concordance.csv}.
...
This subsection makes the synthesis explicit by mapping each target to the evidence
axes that were supported, unresolved, not evaluated, or not applicable; evidence
completeness is target-specific. Supplementary
Table~\ref{tab:supp-evidence-axis-audit} is the row-level audit behind main-text Figure 7:
it links every target--axis state to its claim source, current interpretation, and the
evidence required to change that state. A supported state applies only to its named axis;
not evaluated and not applicable are not negative results. The table is therefore a map of
qualified claims and open gaps, not a scorecard in which one supported axis compensates for
...
\input{generated/stable5_analysis_frame_inventory.tex}
\end{landscape}
...
\input{generated/stable6_stability_contrast_summary.tex}

Figure S4 and Table S6 make the heterogeneity behind the condensed stability decision
...
\input{generated/stable7_endpoint_concordance.tex}

Figure S5 and Table S7 show why raw agreement alone is insufficient. The exact Official-PFI
...
values. Stability lineage is recorded in \path{resources/projects/prostate_biomarker_validation/model_workspace/stability_run_manifest.csv} and
\path{resources/projects/prostate_biomarker_validation/model_workspace/stability_qc_report.json}; recurrence lineage is recorded in the paired
survival and common-source run configurations and manifests; AR and SPOP source hashes are
recorded in \path{resources/projects/prostate_biomarker_validation/model_workspace/ar_spop_evidence_manifest.csv}. The manuscript source tables
