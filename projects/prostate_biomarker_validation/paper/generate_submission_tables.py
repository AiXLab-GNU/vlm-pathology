...
def _source_path(source_root: Path, relative: str | Path) -> Path:
    ...
    if path.parts and path.parts[0] == "paper":
        return Path(source_root) / "projects/prostate_biomarker_validation/paper" / Path(
            *path.parts[1:]
        )
    return candidate
...
def generate_submission_tables(
    ...
    for spec in specs:
        ...
        frames = [pd.read_csv(_source_path(source_root, source)) for source in spec.sources]
