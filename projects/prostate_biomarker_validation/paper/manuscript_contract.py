...
def _submission_text(root: Path) -> str:
    canonical = root / "projects/prostate_biomarker_validation/paper/main.tex"
    staged = root / "paper/main.tex"
    return _expand_inputs(canonical if canonical.is_file() else staged, root)
...
def validate_manuscript(root: Path) -> list[ContractResult]:
    ...
    results = [
        ...
        ContractResult(
            ...
            "required declarations: Data Availability, Code Availability, Funding, "
            "Author Contributions, Competing Interests, and ethics/consent",
        ),
    ]
