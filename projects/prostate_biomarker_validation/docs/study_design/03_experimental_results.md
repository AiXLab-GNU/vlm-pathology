...
실행했다(`resources/projects/prostate_biomarker_validation/model_workspace/pilot_confounder_audit.py`, TCGA-PRAD 300슬라이드, 통계 보정과
...
**Grade-내부 permutation 검정** (`resources/projects/prostate_biomarker_validation/model_workspace/pilot_confounder_permutation.py`, 2026-07-31):
...
`resources/projects/prostate_biomarker_validation/model_workspace/pilot_tcga_multiassay_concordance.py`. cBioPortal REST API로 PTEN/ERG mRNA
...
508명(Radboudumc, 등록 없이 공개 S3에서 확보 — `resources/data/shared/opendataset/LEOPARD/`) 대상,
...
재현: `resources/projects/prostate_biomarker_validation/model_workspace/pilot_conch_leopard_embed.py`/`pilot_virchow_leopard_embed.py`
...
**라벨 재구축**: `resources/data/shared/opendataset/TCGA-PRAD-BCR/bcr.csv`(기존, 출처 불명)를 GDC
...
재현: `resources/data/shared/opendataset/TCGA-PRAD-BCR/build_bcr_labels.py`(라벨),
`resources/projects/prostate_biomarker_validation/model_workspace/pilot_tcga_prad_recurrence_external.py`(CONCH 검증),
`resources/projects/prostate_biomarker_validation/model_workspace/pilot_tcga_prad_recurrence_external_virchow.py`(Virchow 검증, 스케일불일치),
`resources/projects/prostate_biomarker_validation/model_workspace/pilot_virchow_tcga_prad_recurrence_scalematch.py`(스케일매칭 재임베딩).
...
재현: `resources/projects/prostate_biomarker_validation/model_workspace/pilot_precise_spatial_facevalidity.py`(150μm 버전이 최종,
...
재현: `resources/projects/prostate_biomarker_validation/model_workspace/pilot_qwk_comparison.py`. `report.tex` "QWK 재계산과 SOTA
비교" 절.
...
존재 확인 — GDC API·cBioPortal API 직접 쿼리로 검증, `resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_clinical_extra/`에
원본 JSON 보존, `resources/projects/prostate_biomarker_validation/model_workspace/fetch_tcga_prad_clinical_extra.py`로 재현 가능)까지 포함한
완전조정 모델로 확장(`resources/projects/prostate_biomarker_validation/model_workspace/pilot_marker7_confounder_audit.py`
...
재현: `resources/projects/prostate_biomarker_validation/model_workspace/pilot_marker7_confounder_audit.py`(확장 함수),
`resources/projects/prostate_biomarker_validation/model_workspace/marker7_extended_confounder_summary.csv`. `paper/sections/results.tex`
...
GDC-재구축 라벨(§6e)을 벤치마크(`resources/projects/prostate_biomarker_validation/model_workspace/pilot_tcga_prad_label_benchmark.py`).
...
재현: `resources/projects/prostate_biomarker_validation/model_workspace/pilot_tcga_prad_label_benchmark.py`,
`resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_label_benchmark_summary.csv`. `paper/sections/results.tex`
§res-label-benchmark.

### 8c. AR(마커⑥) 사이트별 forest plot + bootstrap CI (Tier-1 1.3)

§3b의 leave-one-site-out 점추정(사이트별 ρ)에 환자-클러스터 부트스트랩 95% CI(2,000회,
사이트별 재실행)를 추가(`resources/projects/prostate_biomarker_validation/model_workspace/pilot_ar_site_forest.py`). 결과: CH −0.13
...
재현: `resources/projects/prostate_biomarker_validation/model_workspace/pilot_ar_site_forest.py`, `resources/projects/prostate_biomarker_validation/model_workspace/ar_site_forest_summary.csv`,
...
기존 `resources/projects/prostate_biomarker_validation/model_workspace/statistical_corrections_summary.csv`(13행 전부 이미 존재)를 신규
...
LEOPARD 3-pool(`resources/projects/prostate_biomarker_validation/model_workspace/leopard_conch_cache/pool_comparison_summary.csv`, 이미
...
유효할 수 있음, 승인 미확보, organizer 문의 초안(`resources/data/shared/opendataset/LEOPARD/organizer_email_draft.txt`)
...
(`resources/projects/prostate_biomarker_validation/model_workspace/pilot_spop_classweight_ablation.py`): slide AUROC 0.508 vs 0.506, patient
...
loss)에 적용(`resources/projects/prostate_biomarker_validation/model_workspace/pilot_tcga_prad_scale_tile_sensitivity.py`). **범위 조정**: 최초
...
임베딩(`resources/projects/prostate_biomarker_validation/model_workspace/virchow_tcga_prad_cache/X_spop.npy`, 같은 300슬라이드/273환자
universe, file_name/case_id 순서 직접 대조로 CONCH 캐시와 동일함 확인)으로
재실행했다(`resources/projects/prostate_biomarker_validation/model_workspace/pilot_confounder_audit_nested_virchow.py`, CPU 전용, 재임베딩
...
히트맵(신규, `resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_scale_tile_sensitivity_summary.csv`의 AR/SPOP 컬럼
...
`resources/data/shared/song-datasets/_previews/latex/report.tex` §9.6, §10에 있음(주 기록 문서). 이 문서는
그 요약본이다.
