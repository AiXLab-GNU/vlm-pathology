...
> 벤치마크") 참고, 재현 스크립트는 `resources/projects/prostate_biomarker_validation/model_workspace/pilot_vlm_benchmark_{gpt5,claude,quilt,
...
3. **Quilt-LLaVA** — 이미 완전히 세팅됨: `resources/projects/prostate_biomarker_validation/model_workspace/.venv-quilt`, `resources/projects/prostate_biomarker_validation/model_workspace/Quilt-Llava-v1.5-7b`,
   기존 파일럿 스크립트 `resources/projects/prostate_biomarker_validation/model_workspace/pilot_quilt_llava_*.py` (재사용/참고)
4. **LLaVA-Med** (Microsoft, 오픈소스 의료 특화 VLM) — 아직 미설치, GitHub
   `microsoft/LLaVA-Med` 또는 HuggingFace에서 받아서 설치 필요 (Quilt-LLaVA 설치
   방식 `resources/projects/prostate_biomarker_validation/model_workspace/download_quilt_llava.sh` 참고해서 유사하게 진행)
...
`resources/data/shared/opendataset/SICAPv2` 재사용 (이미 있는 데이터, 새 다운로드 불필요):
...
- 원본 응답 로그도 저장(감사 가능하도록), `resources/projects/prostate_biomarker_validation/model_workspace/pilot_vlm_benchmark_*.py` 형태의
  재현 가능한 스크립트로 작성.
- 결과를 `resources/data/shared/song-datasets/_previews/latex/report.tex`의 VLM 관련 절(§5.2 또는 적절한
