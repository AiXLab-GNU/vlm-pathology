...
sys.path.insert(0, str(ROOT / "resources/projects/prostate_biomarker_validation/model_workspace"))
...
def build_folds() -> pd.DataFrame:
    parts = []
    nadt_grade = pd.read_csv(ROOT / "resources/projects/prostate_biomarker_validation/model_workspace/nadt_conch_cache/meta.csv")
    parts.append(assign_group_folds("gleason", "NADT-Prostate", nadt_grade["patient_id"].values))

    nadt_pheno = pd.read_csv(ROOT / "resources/projects/prostate_biomarker_validation/model_workspace/nadt_conch_cache/meta_phenotype.csv")
    ...
    spop = pd.read_csv(ROOT / "resources/projects/prostate_biomarker_validation/model_workspace/virchow_tcga_prad_cache/meta_spop.csv")
...
def main() -> None:
    spec = build_spec()
    folds = build_folds()
    spec.to_csv(ROOT / "resources/projects/prostate_biomarker_validation/model_workspace/stability_grid_spec.csv", index=False)
    folds.to_csv(ROOT / "resources/projects/prostate_biomarker_validation/model_workspace/stability_fold_assignments.csv", index=False)
