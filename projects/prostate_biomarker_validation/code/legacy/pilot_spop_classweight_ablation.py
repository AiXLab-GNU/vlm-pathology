...
Run with the CONCH-only venv (fast, CPU only, reuses cached embeddings):
    HF_HOME=~/.cache/huggingface-jhkim resources/projects/prostate_biomarker_validation/model_workspace/.venv-conch/bin/python \
        resources/projects/prostate_biomarker_validation/model_workspace/pilot_spop_classweight_ablation.py
"""
import os
import sys
...
def main():
    ...
    pd.DataFrame(rows).to_csv(
        os.path.join(ROOT, "resources/projects/prostate_biomarker_validation/model_workspace/spop_classweight_ablation_summary.csv"), index=False)
    print(f"\nsaved resources/projects/prostate_biomarker_validation/model_workspace/spop_classweight_ablation_summary.csv")
