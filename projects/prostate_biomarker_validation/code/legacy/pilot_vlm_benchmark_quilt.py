"""Formal VLM hallucination benchmark -- Quilt-LLaVA (local, already-installed model).
...
Supersedes/formalizes the three earlier ad hoc pilots (pilot_quilt_llava_{ranking,
conversation,forced_pairwise}.py) with the same task battery + rubric used for the other
3 models, on the new SICAPv2-Test-split stratified set instead of the old GNUH IHC crops.

Run with the VLM-only venv:
    resources/projects/prostate_biomarker_validation/model_workspace/.venv-quilt/bin/python resources/projects/prostate_biomarker_validation/model_workspace/pilot_vlm_benchmark_quilt.py --set pilot
    resources/projects/prostate_biomarker_validation/model_workspace/.venv-quilt/bin/python resources/projects/prostate_biomarker_validation/model_workspace/pilot_vlm_benchmark_quilt.py --set full
"""
import argparse
import json
import os
...
CONV_MODE = "llava_v1"
OUT_DIR = os.path.join(CACHE_DIR, "quilt")
...
def main():
    ...
    print(f"Running Quilt-LLaVA benchmark on '{args.set}' set: {len(df)} images")
