...
LEOPARD = ROOT / "resources/data/shared/opendataset/LEOPARD/training"
BCR = ROOT / "resources/data/shared/opendataset/TCGA-PRAD-BCR/bcr.csv"
...
def frames(encoder):
    source = pd.read_csv(ROOT / f"resources/projects/prostate_biomarker_validation/model_workspace/leopard_{encoder.lower()}_cache/meta.csv")
    source["file_name"] = source.case_id
    source["path"] = source.file_name.map(lambda x: str(LEOPARD / x))
    target = pd.read_csv(ROOT / "resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_conch_cache/meta.csv")[["file_name", "case_id"]]
...
def main():
    ...
    out=ROOT/"resources/projects/prostate_biomarker_validation/model_workspace/stability_runs"/a.output_tag/f"marker7_{a.encoder.lower()}"; out.mkdir(parents=True,exist_ok=True)
