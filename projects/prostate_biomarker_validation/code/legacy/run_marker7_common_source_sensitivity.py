...
``resources/projects/prostate_biomarker_validation/model_workspace/.venv-conch`` but is not installed in the workspace environment.  To
...
def validate_r1_config_lineage(
    ...
    for snapshot in snapshots.values():
        ...
        if "stability_runs" not in parts or not Path(artifact_path).name.startswith(prefixes):
            continue
        # R1 is immutable and used the former root-level ``models/`` logical
        # namespace. New snapshots use the canonical project-owned resource
        # path, so lineage reconciliation accepts that one exact historical
        # alias without reintroducing a filesystem compatibility link.
        lookup_path = artifact_path
        resource_prefix = Path(
            "resources/projects/prostate_biomarker_validation/model_workspace"
        ).parts
        if lookup_path not in declared.index and parts[: len(resource_prefix)] == resource_prefix:
            historical = Path("models", *parts[len(resource_prefix) :]).as_posix()
            if historical in declared.index:
                lookup_path = historical
        if lookup_path not in declared.index:
            raise R2IntegrityError(f"R1 lineage is missing frozen input {artifact_path}")
        row = declared.loc[lookup_path]
...
def configuration_embedding_lineage_sha256(
    ...
    for snapshot in snapshots.values():
        ...
        if "stability_runs" in Path(artifact_path).parts and Path(artifact_path).name.startswith(prefixes):
            parts = Path(artifact_path).parts
            lineage_path = artifact_path
            if parts[: len(resource_prefix)] == resource_prefix:
                lineage_path = Path(
                    "models", *parts[len(resource_prefix) :]
                ).as_posix()
            records.append((lineage_path, str(snapshot["sha256"])))
    records.sort()
...
def _worker_run(job_path: Path, output_path: Path) -> None:
    probe = worker_probe()
    validate_worker_probe(probe, ROOT / "resources/projects/prostate_biomarker_validation/model_workspace/.venv-conch/bin/python")
...
def _input_paths_and_roles(root: Path, configs: list[SourceConfig]) -> tuple[list[Path], dict[Path, str]]:
    ...
    supporting = {
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_grid_spec.csv": "frozen_spec",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_fold_assignments.csv": "frozen_folds",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_cell_results.csv": "r1_integrated_cells",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_contrast_summary.csv": "r1_contrasts",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_qc_report.json": "r1_qc",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_tile_coordinate_manifest.csv": "r1_coordinate_manifest",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_run_manifest.csv": "r1_run_manifest",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_runs/full/marker7_conch/cell_results.csv": "raw_cells:marker7_conch",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_runs/full/marker7_conch/fold_results.csv": "raw_folds:marker7_conch",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_runs/full/marker7_virchow/cell_results.csv": "raw_cells:marker7_virchow",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_runs/full/marker7_virchow/fold_results.csv": "raw_folds:marker7_virchow",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/run_stability_marker7.py": "frozen_runner",
...
def run_analysis(*, root: Path = ROOT, output_dir: Path | None = None) -> dict[str, Path]:
    ...
    output_dir = (Path(output_dir) if output_dir is not None else root / "resources/projects/prostate_biomarker_validation/model_workspace").resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    run_root = root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_runs/full"
    configs = discover_configs(run_root)
    input_paths, input_roles = _input_paths_and_roles(root, configs)
    before = snapshot_paths(input_paths, root)
    r1_manifest_path = (root / "resources/projects/prostate_biomarker_validation/model_workspace/stability_run_manifest.csv").resolve()
    ...
    interpreter = root / "resources/projects/prostate_biomarker_validation/model_workspace/.venv-conch/bin/python"
    worker = probe_worker(interpreter)
    controller_record = {**controller, "executable": ".venv/bin/python"}
    worker_record = {**worker, "executable": "resources/projects/prostate_biomarker_validation/model_workspace/.venv-conch/bin/python"}
    ...
    with tempfile.TemporaryDirectory(prefix="marker7-common498-", dir=output_dir.parent) as temporary:
            ...
            "configs": [
                {
                    ...
                    "raw_cells": f"resources/projects/prostate_biomarker_validation/model_workspace/stability_runs/full/marker7_{config.encoder_key}/cell_results.csv",
                }
        ...
        official_output = output_dir == (root / "resources/projects/prostate_biomarker_validation/model_workspace").resolve()
        labels = {
            path.resolve(): (f"resources/projects/prostate_biomarker_validation/model_workspace/{name}" if official_output else name)
