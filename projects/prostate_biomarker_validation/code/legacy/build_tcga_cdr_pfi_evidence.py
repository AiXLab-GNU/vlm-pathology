...
def _official_source(root: Path) -> tuple[pd.DataFrame, dict]:
    directory = root / "resources/data/prostate_biomarker_validation/local-data/TCGA-CDR"
...
def _clinical_json_predictions(root: Path, risk: pd.DataFrame, endpoint_id: str) -> pd.DataFrame:
    ...
    data = json.loads(
        (root / "resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_clinical_extra/prad_pancan_clinical.json").read_text(
            encoding="utf-8"
        )
    )
...
def _recurrence_only_predictions(root: Path, risk: pd.DataFrame) -> pd.DataFrame:
    source = pd.read_csv(root / "resources/data/shared/opendataset/TCGA-PRAD-BCR/bcr_recurrence_only.csv")
...
def _input_contract(root: Path) -> tuple[list[Path], dict[Path, str]]:
    roles = {
        root / f"resources/data/prostate_biomarker_validation/local-data/TCGA-CDR/{EXPECTED_OFFICIAL_FILENAME}": "official_tcga_cdr_table_s1",
        root / "resources/data/prostate_biomarker_validation/local-data/TCGA-CDR/PanCan-Clinical_Open_GDC-Manifest_1.txt": "official_gdc_manifest",
        root / "resources/data/prostate_biomarker_validation/local-data/TCGA-CDR/source_provenance.json": "official_source_provenance",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/confounder_nested_predictions.csv": "frozen_marker7_risk",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_clinical_extra/prad_pancan_clinical.json": "pfs_dfs_comparator",
        root / "resources/data/shared/opendataset/TCGA-PRAD-BCR/bcr_recurrence_only.csv": "recurrence_only_comparator",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/build_tcga_cdr_pfi_evidence.py": "analysis_entry_point",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/pilot_tcga_prad_label_benchmark.py": "historical_endpoint_benchmark",
        root / "projects/prostate_biomarker_validation/paper/MajorRevision-v1-remaining-experiments-and-revision-plan.md": "approved_R3_card",
...
def validate_run_parameters(root: Path, output_dir: Path, *, n_boot: int, seed: int) -> None:
    ...
    canonical_output = (Path(root).resolve() / "resources/projects/prostate_biomarker_validation/model_workspace").resolve()
...
def run_analysis(
    ...
    risk_path = root / "resources/projects/prostate_biomarker_validation/model_workspace/confounder_nested_predictions.csv"
    ...
    with tempfile.TemporaryDirectory(prefix=".tcga-cdr-pfi-stage-", dir=output_dir.parent) as temporary:
        ...
        config = {
            ...
            "frozen_risk": {
                "source": "resources/projects/prostate_biomarker_validation/model_workspace/confounder_nested_predictions.csv",
