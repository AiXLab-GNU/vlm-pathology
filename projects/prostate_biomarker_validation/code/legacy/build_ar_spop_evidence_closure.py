...
def load_cohort(root: Path) -> tuple[np.ndarray, pd.DataFrame, pd.DataFrame]:
    ...
    meta_path = root / "resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_conch_cache/meta.csv"
    embedding_path = root / "resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_conch_cache/X.npy"
    clinical_path = root / "resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_clinical_extra/prad_pub_sample_clinical.json"
    manifest_path = root / "resources/data/shared/opendataset/TCGA-PRAD/manifest.csv"
...
def build_metadata_availability(root: Path, slides: pd.DataFrame, gdc: pd.DataFrame) -> pd.DataFrame:
    ...
    for record in slides.itertuples(index=False):
        path = root / "resources/data/shared/opendataset/TCGA-PRAD/slides" / record.file_name
        if not path.is_file():
...
def build_manifest(
    ...
    for record in slide_metadata.itertuples(index=False):
        rows.append({
            ...
            "artifact_path": f"resources/data/shared/opendataset/TCGA-PRAD/slides/{record.file_name}",
...
def _input_contract(root: Path) -> tuple[list[Path], dict[Path, str]]:
    roles = {
        root / "resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_conch_cache/X.npy": "frozen_conch_embeddings",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_conch_cache/meta.csv": "frozen_cache_metadata",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/tcga_prad_clinical_extra/prad_pub_sample_clinical.json": "clinical_labels",
        root / "resources/data/shared/opendataset/TCGA-PRAD/manifest.csv": "gdc_manifest",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/ar_site_forest_summary.csv": "frozen_ar_forest",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/spop_classweight_ablation_summary.csv": "frozen_spop_ablation",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/pilot_tcga_prad_site_split.py": "historical_site_runner",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/pilot_ar_site_forest.py": "historical_ar_runner",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/pilot_spop_classweight_ablation.py": "historical_spop_runner",
        root / "resources/projects/prostate_biomarker_validation/model_workspace/build_ar_spop_evidence_closure.py": "analysis_entry_point",
        ...
        root / "projects/prostate_biomarker_validation/paper/MajorRevision-v1-remaining-experiments-and-revision-plan.md": "approved_R5_design",
    }
...
def run_analysis(root: Path = ROOT, output_dir: Path | None = None) -> dict[str, Path]:
    ...
    output_dir = root / "resources/projects/prostate_biomarker_validation/model_workspace" if output_dir is None else Path(output_dir).absolute()
    ...
    forest = pd.read_csv(root / "resources/projects/prostate_biomarker_validation/model_workspace/ar_site_forest_summary.csv")
    ...
    slide_paths = [root / "resources/data/shared/opendataset/TCGA-PRAD/slides" / name for name in slides.file_name]
