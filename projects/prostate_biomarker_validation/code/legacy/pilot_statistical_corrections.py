...
Run with the CONCH-only venv (fast -- CPU only, no embedding, just probe refits + bootstrap):
    HF_HOME=~/.cache/huggingface-jhkim resources/projects/prostate_biomarker_validation/model_workspace/.venv-conch/bin/python \
        resources/projects/prostate_biomarker_validation/model_workspace/pilot_statistical_corrections.py
"""
import json
import os
...
def main():
    ...
    manifest = pd.read_csv(os.path.join(ROOT, "resources/data/shared/opendataset/TCGA-PRAD/manifest.csv"))
    ...
    out_path = os.path.join(ROOT, "resources/projects/prostate_biomarker_validation/model_workspace/statistical_corrections_summary.csv")
