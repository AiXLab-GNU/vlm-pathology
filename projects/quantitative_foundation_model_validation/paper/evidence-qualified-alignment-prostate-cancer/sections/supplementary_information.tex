...
a disease-prediction head functionally uses the target. Grade alone has a separate internal
exploratory locked-head erasure analysis; it is not an external, indispensable, mechanistic,
or clinical-increment result. No clinician trust, reliance, diagnostic-performance, or
...
The main study combined five resources for complementary purposes rather than pooling them
...
LEOPARD supplied the source recurrence analysis. Patient, case-image, session, and slide
units were never treated as interchangeable.

Table~\ref{tab:supp-clinical-reference-primer} preserves the detailed clinical, reference,
...
Recurrence & Time from treatment to a defined return or progression event. & LEOPARD and
TCGA time-to-event references; event definitions were available but non-equivalent. &
Censored time-to-event target; C-index for ordering and IBS for prediction error. \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:supp-resource-orientation} records why the five resources and the correlated
...
LEOPARD & Post-hoc recurrence risk score fitted in the source cohort & Transfer without
refitting to the TCGA-PRAD target frames \\
...
Table~\ref{tab:supp-target-registry} is the reading key for all subsequent analyses: it
...
\input{generated/stable1_target_registry.tex}

Table~\ref{tab:supp-six-axis-evidence-stage} expands the Main evidence-state summary by
...
\midrule
Grade/ISUP & Architectural differentiation and aggressiveness & Best-qualified recoverability and transport; CONCH and Virchow internal ISUP directions & Internal exploratory BCR-head sensitivity demonstrated \\
...
Table~\ref{tab:supp-primary-estimates} reports the complete primary recoverability and
...
\input{generated/stable2_primary_estimates.tex}
...
functional use, external functional transport, and strong mechanistic alignment remain
unsupported.
...
Table~\ref{tab:supp-conditional-increments} makes the conditional comparison explicit:
...
\input{generated/stable3_conditional_increments.tex}
...
Table~\ref{tab:supp-site-audits} retains every AR and SPOP site row, including the undefined
...
\input{generated/stable_site_audits.tex}
...
Figure~\ref{fig:supp-full-grid} shows every configuration mean relative to its
target-specific null. Table~\ref{tab:supp-stability} then condenses those same correlated
settings into ranges and null-straddling counts. The Main manuscript displays that summary
graphically and also shows the seed-matched setting contrasts. These views describe the same
sensitivity program and are not independent replications.
...
\input{generated/stable4_stability_summary.tex}
...
Table~\ref{tab:supp-contrast-summary} summarizes every target within each registered contrast
...
\input{generated/stable_contrast_summary.tex}

The Main manuscript visualizes the same paired contrasts and preserves their direction:
...
Table~\ref{tab:supp-endpoints} defines the endpoint IDs, their roles, metrics, and the
...
The comparison names in Tables~\ref{tab:supp-outcome-reconstructed} and
...
\input{generated/stable5_endpoint_hierarchy.tex}


Tables~\ref{tab:supp-outcome-reconstructed} and
...
\input{generated/stable_outcome_reconstructed.tex}


\input{generated/stable_outcome_official_pfi.tex}

The Main manuscript plots the paired intervals from the two tables on a common
positive-favors-first-or-augmented convention; endpoint-specific panels remain separate
because their event definitions and counts differ.
...
reconstructed from prose. Table~\ref{tab:supp-missingness-summary} instead reports every
...
\input{generated/stable_missingness_summary.tex}
...
Table~\ref{tab:supp-claim-matrix} exposes the manuscript claim ID, source-claim lineage,
...
\input{generated/stable6_claim_matrix.tex}
...
For the FM6 extension specifically, clean-rerun handoff required 20/20 nonvolatile output
hashes, unchanged headline numbers, and unchanged gates. Both encoders retained
R=whole-tissue recoverability, A=internal disease association, and U=exploratory
whole-tissue functional sensitivity; T=external transport remained not tested and strong H2
remained prohibited. These gate labels describe evidence scope rather than a sequence of
...
includes a locked BCR head and target-erasure experiment, but only as whole-tissue internal
exploratory evidence. Its detector did not pass the independent-domain sensitivity gate,
refitted heads could compensate, and the limited ISUP-only comparison was not a sufficient
clinical-increment study. The present paper therefore establishes candidate shared
interpretive coordinates and scoped internal grade sensitivity, not complete explanation of
AI decisions. Residual or unknown-feature marker discovery is reserved for a separate study.
