...
Run with the CONCH-only venv on a free GPU:
    CUDA_VISIBLE_DEVICES=<free_gpu> HF_HOME=~/.cache/huggingface-jhkim \
        models/.venv-conch/bin/python models/pilot_tcga_prad_scale_tile_sensitivity.py
"""
import json
import os
import re
...
TCGA_ROOT = os.path.join(ROOT, "opendataset/TCGA-PRAD")
SLIDE_DIR = os.path.join(TCGA_ROOT, "slides")
CACHE_META = os.path.join(ROOT, "models/tcga_prad_conch_cache/meta.csv")
CBIOPORTAL_SAMPLE_JSON = os.path.join(
    ROOT, "models/tcga_prad_clinical_extra/prad_pub_sample_clinical.json")
OUT_CSV = os.path.join(ROOT, "models/tcga_prad_scale_tile_sensitivity_summary.csv")
