...
PROVENANCE_RELATIVE_PATHS = (
    "models/aggregate_stability_grid.py", "models/build_stability_grid_spec.py",
    "models/run_stability_tcga.py", "models/run_stability_nadt.py",
    "models/run_stability_marker7.py", "RUN_REPRODUCTION.md", "environment.yml",
...
def _derive_nadt_tiff_paths(coordinate_manifest: pd.DataFrame) -> list[Path]:
    ...
    for value in nadt["raw_metadata_path"].dropna().unique():
        ...
        for raw_path in metadata["path"]:
            paths.add(Path(_nonblank(raw_path, "NADT TIFF path")))
...
def _derive_nadt_tiff_paths_from_declared_inputs(paths: Sequence[Path]) -> list[Path]:
    ...
    for metadata_path in metadata_paths:
        ...
        for value in metadata["path"]:
            result.add(Path(_nonblank(value, "NADT TIFF path")))
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--spec", type=Path, default=Path("models/stability_grid_spec.csv"))
    parser.add_argument(
        "--fold-assignments", type=Path,
        default=Path("models/stability_fold_assignments.csv"),
    )
    parser.add_argument("--run-root", type=Path, default=Path("models/stability_runs/full"))
    parser.add_argument("--log-root", type=Path, default=Path("models/stability_runs/logs"))
    parser.add_argument("--output-dir", type=Path, default=Path("models"))
    parser.add_argument("--figure-data-dir", type=Path, default=Path("paper/figure_data"))
