...
models/statistical_corrections_summary.csv (5-fold patient-disjoint internal CV across all
...
Run with the CONCH-only venv (fast, CPU only, reuses cached embeddings):
    HF_HOME=~/.cache/huggingface-jhkim models/.venv-conch/bin/python \
        models/pilot_ar_site_forest.py
"""
import os
import sys
...
N_BOOT = 2000
SEED = 0
OUT_CSV = os.path.join(ROOT, "models/ar_site_forest_summary.csv")
STAT_CORR_CSV = os.path.join(ROOT, "models/statistical_corrections_summary.csv")
