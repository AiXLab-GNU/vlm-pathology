...
labels (opendataset/TCGA-PRAD-BCR/bcr.csv).
...
models/tcga_prad_clinical_extra/*.json (verified directly, not assumed from field names, per
...
Run with the CONCH-only venv:
    HF_HOME=~/.cache/huggingface-jhkim models/.venv-conch/bin/python \
        models/pilot_marker7_confounder_audit.py
"""
import json
import os
import sys
...
CLINICAL_EXTRA_DIR = os.path.join(ROOT, "models/tcga_prad_clinical_extra")
N_PERM = 2000
SEED = 0
...
def main():
    ...
    pd.DataFrame([ext]).to_csv(
        os.path.join(ROOT, "models/marker7_extended_confounder_summary.csv"), index=False)
    print(f"\nsaved models/marker7_extended_confounder_summary.csv")
