...
def load_reconstructed_common_cohort(root: Path = ROOT) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    return reconcile_common_cohort(
        pd.read_csv(root / "models/confounder_nested_predictions.csv"),
        pd.read_csv(root / "models/marker7_clinical_hierarchy_predictions.csv"),
        pd.read_csv(root / "models/stability_fold_assignments.csv"),
        pd.read_csv(root / "opendataset/TCGA-PRAD-BCR/bcr.csv"),
        expected_n=COMMON_N,
...
def run_analysis(*, root: Path = ROOT, output_dir: Path | None = None) -> dict[str, Path]:
    ...
    worker = root / "models/.venv-conch/bin/python"
