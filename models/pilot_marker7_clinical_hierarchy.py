...
def main():
    ...
    summary.to_csv(os.path.join(ROOT, "models/marker7_clinical_hierarchy_summary.csv"), index=False)
    pd.DataFrame(missingness_rows).to_csv(
        os.path.join(ROOT, "models/marker7_clinical_hierarchy_missingness.csv"), index=False)
    predictions.to_csv(
        os.path.join(ROOT, "models/marker7_clinical_hierarchy_predictions.csv"), index=False)
    delta_row.to_csv(
        os.path.join(ROOT, "models/marker7_clinical_hierarchy_delta.csv"), index=False)
