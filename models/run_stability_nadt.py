...
NADT = ROOT / "opendataset/NADT-Prostate_v1"
FOLDS = ROOT / "models/stability_fold_assignments.csv"


def load_frame():
    grade = pd.read_csv(ROOT / "models/nadt_conch_cache/meta.csv").rename(
        columns={"gleason_total": "gleason"})[["file_name", "patient_id", "gleason"]]
    phenotype = pd.read_csv(ROOT / "models/nadt_conch_cache/meta_phenotype.csv").rename(
...
def main():
    ...
    out = ROOT / "models/stability_runs" / a.output_tag / f"nadt_{a.encoder.lower()}"
