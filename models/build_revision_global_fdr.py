...
def main():
    marker = pd.read_csv(os.path.join(ROOT, "models/statistical_corrections_summary.csv"))
    ...
    audit = pd.read_csv(os.path.join(
        ROOT, "models/confounder_refit_permutation_final2000_summary.csv"))
    ...
    family.to_csv(os.path.join(ROOT, "models/revision_global_fdr_summary.csv"), index=False)
    marker["patient_q_BH_FDR_17_tests"] = family.loc[
        family["family_member_type"] == "marker_hypothesis",
        "q_value_BH_FDR_17_tests"].to_numpy()
    marker.to_csv(os.path.join(ROOT, "models/statistical_corrections_summary.csv"), index=False)
