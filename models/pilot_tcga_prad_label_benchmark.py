...
Our label (opendataset/TCGA-PRAD-BCR/bcr.csv, built by build_bcr_labels.py) is derived from GDC
...
Run with the CONCH-only venv:
    HF_HOME=~/.cache/huggingface-jhkim models/.venv-conch/bin/python \
        models/pilot_tcga_prad_label_benchmark.py
"""
import json
import os
...
LEOPARD_CACHE = os.path.join(ROOT, "models/leopard_conch_cache")
BCR_CSV = os.path.join(ROOT, "opendataset/TCGA-PRAD-BCR/bcr.csv")
RECURRENCE_ONLY_CSV = os.path.join(
    ROOT, "opendataset/TCGA-PRAD-BCR/bcr_recurrence_only.csv")
PANCAN_JSON = os.path.join(ROOT, "models/tcga_prad_clinical_extra/prad_pancan_clinical.json")
...
def main():
    ...
    pd.DataFrame(rows).to_csv(
        os.path.join(ROOT, "models/tcga_prad_label_benchmark_summary.csv"), index=False)
    print(f"\nsaved models/tcga_prad_label_benchmark_summary.csv")
