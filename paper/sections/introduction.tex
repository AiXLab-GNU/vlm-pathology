Pathology foundation models turn whole-slide images (digitized views of complete tissue
sections) into reusable numerical representations from which lightweight probes, or
simple task-specific models, can recover grade, tissue phenotype, molecular labels and
...
The displayed counts identify the unit used by each evidence component. They are
row-specific and must not be summed because patients, slides, folds, and settings can
overlap; no resource alone covered every qualification dimension.}
\begin{tabular}{@{}p{0.20\textwidth}p{0.43\textwidth}p{0.28\textwidth}@{}}
\toprule
Resource or component & Target and saved analysis frame & Role in the qualification design \\
\midrule
NADT-Prostate \cite{nadt2021} & Grade and phenotype; 39 patients & Source-cohort probe
fitting and patient-level evaluation \\
PANDA \cite{panda2022} & Grade: 565 Karolinska and 572 Radboud case images; phenotype:
469 and 483 tumor cases & Cross-cohort transfer without target-cohort refitting \\
PRECISE \cite{precise2026} & Grade; 17 evaluable imaging sessions & Additional resource
with a session-level analysis unit \\
TCGA-PRAD \cite{tcgaprad2015,tcgacdr2018} & Molecular analyses: 273 patients; recurrence
transfer: 270 patients; paired recurrence increment: 153 complete cases & Molecular,
clinical-increment, site, and endpoint audits \\
LEOPARD \cite{leopard2026} & Post-hoc recurrence risk score fitted in the source cohort &
Transfer without refitting to the TCGA-PRAD target frames \\
...
\paragraph{Clinical and analytical orientation.}
For readers across pathology and medical AI, Gleason grading summarizes the gland-forming
architecture of prostate cancer: a pathologist combines the predominant and next most
prevalent patterns, and higher scores or International Society of Urological Pathology
(ISUP) Grade Groups indicate less differentiated morphology. In this study, grade refers to
the source-provided Gleason or ISUP label at the stated patient, case-image, or imaging-session
unit. ``Tumor phenotype'' instead denotes the dataset-specific tumor-content target or the
grade-derived benign-versus-tumor label; it is not a molecular subtype
\cite{panda2022}.

The molecular targets represent different aspects of prostate-cancer biology. PTEN loss is
loss of a tumor-suppressor signal, SPOP mutation is a recurrent genomic alteration, and AR
activity is a continuous measure of androgen-receptor-regulated transcription. They are
molecular labels evaluated for association with histology-derived representations, not
claims that routine morphology directly diagnoses the underlying alteration. Recurrence
endpoints describe time to a dataset-defined progression or return event; reconstructed
recurrence, progression-free measures, and official progression-free interval are therefore
kept separate \cite{tcgaprad2015,tcgacdr2018}.

Analytically, Spearman correlation measures preservation of continuous or ordinal ranking,
area under the receiver operating characteristic curve (AUROC) measures binary
discrimination, and the concordance index measures time-to-event risk ordering. These
metrics quantify association or discrimination; they do not by themselves establish
calibration, causal interpretation, or readiness for clinical use. A ``frozen'' encoder is
not fine-tuned in this study; a probe is a lightweight
model fitted to its fixed representations, and transfer without refitting means applying
that source-cohort probe unchanged to a target cohort.

In the image workflow, a tile is a small crop sampled from a whole-slide image, and an
embedding is the numerical feature vector produced for that crop. The tile budget is the
number of crops sampled, while the sampling seed fixes the random selection so that it can
be reproduced. A held-out estimate is computed on patients not used to fit the evaluated
model, and patient-disjoint folds keep all material from one patient in the same data
split. A null denotes the no-association or chance benchmark. A patient-bootstrap interval
repeatedly resamples patients to describe statistical uncertainty, whereas a complete-case
analysis includes only patients with every variable required for that comparison.
