...
recurrence event and follow-up labels for recurrence-model fitting. Here, copy-number loss
means fewer genomic copies than the reference state, and biochemical recurrence denotes a
post-treatment rise in prostate-specific antigen under the source study's event definition.
The public NADT,
...
Each tissue-qualified tile, a small image crop, was converted to an embedding, or numerical
feature vector, and the tile embeddings were averaged to form a slide representation. Where the
...
alpha grid. Ridge regression is linear regression with coefficient shrinkage. Binary
TCGA-PRAD screening tests used standardized logistic regression, which estimates a binary
class probability, with fixed regularization to limit overly large fitted coefficients and
with balanced class weighting. The frozen NADT phenotype probe used for
PANDA transfer was instead \texttt{LogisticRegression}\allowbreak\texttt{(C=1.0)} with 2,000 maximum iterations
...
For zero-shot transfer, meaning direct application to a new cohort, the source-cohort probe
and preprocessing were applied to the target cohort without refitting. Native refits and
held-out site analyses were reported as distinct
...
The Mann--Whitney test compares score distributions between two groups; the permutation
test constructs a no-association reference by shuffling labels within the stated strata;
and the Benjamini--Hochberg procedure controls the expected proportion of false discoveries
among the hypotheses declared significant.
...
This nested evaluation separates inner model construction from outer testing; an out-of-fold
prediction is generated by a model that was not fitted using that patient's data.
...
intervals: the Student-$t$ interval summarizes variation across five seed-specific results,
not patient-bootstrap or population-confidence uncertainty. No grid setting was
...
The Cox model relates predictors to relative event hazard while accommodating censored
follow-up, in which the event was not observed before observation ended. PCA compresses
correlated features into mutually orthogonal components.
...
around a slide-level effect; cluster resampling keeps slides from the same patient together.
For legacy NADT/TCGA primary and AR site interval summaries,
...
comparison; such imputation would otherwise replace each missing value with several
model-based draws to propagate missing-data uncertainty.
...
the manuscript work.
A cryptographic hash is a file fingerprint used to detect byte-level change, and a manifest
is the inventory that links those fingerprints to named inputs, code and outputs.
