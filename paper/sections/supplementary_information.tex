...
case-image-, or session-level
estimates and available uncertainty intervals are shown by target and cohort. The graphic
does not imply that an unavailable interval was zero and does not convert aggregate
statistics into reconstructed curves.}
...
Time-dependent AUROC and calibration values are rendered from the saved legacy sources for
the Reconstructed with-tumor endpoint (270 patients; 57 events), not Official TCGA-CDR PFI.
The endpoint name, denominator and event count were fail-closed against the saved
frozen-risk metadata row and the horizon-specific calibration totals.}
...
72 validated configuration means from the saved grid, one target per page. Pages 7--9
display the 390 seed-matched contrasts by comparison type. Null crossings are descriptive
setting checks on reused cohorts and folds.}
...
supported by the frozen analyses. Supported and conditional cells remain bounded to the
named axis; unsupported refers to the frozen primary design, unresolved marks an evaluated
question without sufficient support, and not evaluated or not applicable cells are shown
explicitly rather than treated as negative evidence.}
